function position = update_joystick_data_perturb(position,PERTURBATION_ANGLE)
...
%origin coordsscreen_properties.width_res_pix
origin = position.screen_properties.origin;

%get current position coords
[x_pos_pix,y_pos_pix] = joystick_emulate_mouse(position.screen_properties.joystick_idx,position.screen_properties.joystick_sensitivity);
...
position.error_x_dproj_vec(length(position.error_x_dproj_vec)+1) = (magproj - prev_magproj)*magerror;
