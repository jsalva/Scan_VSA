...
for subject in subject_list:
    ...
    for chunk in range(len(sequence_data)/chunk_size):
        ...
        chunk_time_to_max_vel = chunk_data[chunk_correct,-1]
        ax.plot(np.arange(chunk*chunk_size,(chunk+1)*chunk_size)/chunk_size,chunk_data[:,-1],'o',color='black',alpha='0.2',linestyle='None')
    ...
    indexes = []
    for i in [0,1,2,3]:
        rd_block_1 = sequence_data[(48+16)*i:(48+16)*i+16,-1]
        rd_block_1 = rd_block_1[sequence_data[(48+16)*i:(48+16)*i+16,-2] == 1]      

        sd = sequence_data[(48+16)*i+16:(48+16)*i+16+48,-1]
        sd = sd[sequence_data[(48+16)*i+16:(48+16)*i+16+48,-2] == 1]

        rd_block_2 = sequence_data[(48+16)*i+16+48:(48+16)*i+16+48+16,-1]
        ...
        ax.text(idx_i/8 - 1,3.25+skill_score,'%0.4f'%(skill_score))
        ax.scatter(idx_i/8 - 1,3.2+skill_score,s=100,facecolor='orange')
    x = np.arange(0,idx_i/8+4)
    ax.plot(x,np.repeat(3.2,len(x)),'--',color='orange')

    ax.set_ylabel('Time (s)')
    ax.set_title('%s Sequence Time to Max Velocity'%(subject))
